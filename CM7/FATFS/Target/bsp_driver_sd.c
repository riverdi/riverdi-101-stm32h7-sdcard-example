...
/* Retry init up to 3 times, clearing stale SDMMC command flags before each
 * attempt (CTIMEOUT stays set after card removal and breaks the next CMD8).
 * Starts 1-bit, switches to 4-bit on success. */
uint8_t BSP_SD_Init(void)
{
    ...
    for (uint8_t attempt = 0; attempt < 3; attempt++)
    {
        ...
        hsd1.Init.ClockDiv            = 4;  /* 240 MHz kernel clock / (2*4) = 30 MHz */
